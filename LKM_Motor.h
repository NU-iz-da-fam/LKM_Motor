...
class LKM_Motor {
 public:
  ...
  LKM_Motor(int id, int reduction_ratio, int serial_port, int baudrate);
  void Serial_Init();
  ...
  void Write_Torque_Current(
      double current);  //(10)轉矩閉環控制命令(0xA1), current: -32~32
  ...
  void Write_Angle_MultiRound(
      double angle, double max_speed);  //(13)多圈位置閉環控制命令2(0xA4)
  void Write_Angle_SingleRound(
      double angle,
      bool direction);  //(14)單圈位置閉環控制命令1(0xA5), direction: True ->
                        // 順時針 ; False -> 逆時針
  void Write_Angle_SingleRound(
      double angle, double max_speed,
      bool direction);  //(15)單圈位置閉環控制命令2(0xA6), direction: True ->
                        // 順時針 ; False -> 逆時針
  void Write_Angle_SingleRound(
      double angle);  // 轉向自動-單圈位置閉環控制命令1(往角度小的方向走)
  void Write_Angle_SingleRound(
      double angle,
      double max_speed);  // 轉向自動-單圈位置閉環控制命令2(往角度小的方向走)
  void Write_Angle_Increment(
      double angle_increment);  //(16)增量位置閉環控制命令1(0xA7)
  void Write_Angle_Increment(
      double angle_increment,
      double max_speed);          //(17)增量位置閉環控制命令2(0xA8)
  ...
  // void Read_Setup_Param(uint8_t ParamID); //(24)讀取設定參數命令(0x40) void
  // Read_PID_Param(); //讀取PID參數
  // //(25)寫入設定參數到RAM(0x42), 斷電後失效
  // void Write_Setup_Param_Into_RAM(uint8_t ParamID, uint8_t data1, uint8_t
  // data2, uint8_t data3, uint8_t data4, uint8_t data5, uint8_t data6);
  // //(26)寫入設定參數到ROM(0x44), 斷電後仍然有效
  // void Write_Setup_Param(uint8_t ParamID, uint8_t data1, uint8_t data2,
  // uint8_t data3, uint8_t data4, uint8_t data5, uint8_t data6);
  ...
  void Print_Data();        // 列印出馬達回傳的資料:
                            // 電機溫度、轉矩電流、電機速度以及編碼器位置
  void Print_Angle();       // 列印出馬達回傳的角度
  void Print_PID_Param();   // 列印出馬達的PID參數
  void Set_Need_Receive(
      bool need_receive);  // 設定非讀取資訊的指令是否需要解封包
  void Set_Kt(double Kt);  // 設定馬達的轉矩常數Kt

  int delay_time = 250;    //[us]
  bool debug_mode = true;  // decide whether to use Serial print debug message
                           // in _Receive_Pack() function
 ...
 private:
  ...
  bool _Find_Turn_Direction(
      double target_angle);  // direction: True -> 順時針 ; False -> 逆時針
  double _Normalize_Angle_Deg(double angle);  // 將任意角度轉化成 -180 ~ 180
  // void _Unpack_Read_Setup_Param(uint8_t data_receive[30]);
  // //解讀回傳的設定參數 void _Unpack_Write_Setup_Param(uint8_t
  // data_receive[30]);//解讀設定參數是否成功
  Stream* MOTOR_SERIAL = &Serial1;
  int _id = 0;                       // 馬達設定的id
  int _reduction_ratio = 0;          // 馬達的減速比
  int _serial_port = 0;              // 馬達使用的serial port
  double _Kt = 0.0;                  // 馬達的轉矩常數Kt
  int _baudrate = DEFAULT_BAUDRATE;  // 馬達設定的baudrate
